...
class ulp_vision6_class {
	...
	function popup_options_show($_popup_options) {
						...
						<td>
							...
							<script>
								function ulp_vision6_lists_focus(object) {
									ulp_input_options_focus(object, {"action": "ulp-vision6-lists", "ulp_api_key": jQuery("#ulp_vision6_api_key").val()});
								}
							</script>
						</td>
					</tr>
		...
		if (!empty($popup_options['vision6_api_key']) && !empty($popup_options['vision6_list_id'])) {
			$fields = $this->get_fields_html($popup_options['vision6_api_key'], $popup_options['vision6_list_id'], $popup_options['vision6_fields']);
			echo $fields;
		}
		echo '
							</div>
							...
							<script>
								function ulp_vision6_loadfields() {
									...
									var data = {action: "ulp-vision6-fields", ulp_key: jQuery("#ulp_vision6_api_key").val(), ulp_list: jQuery("#ulp-vision6-list-id").val()};
	...
	function show_fields() {
		...
		if (current_user_can('manage_options')) {
			if (!isset($_POST['ulp_key']) || !isset($_POST['ulp_list']) || empty($_POST['ulp_key']) || empty($_POST['ulp_list'])) {
				$return_object = array();
				$return_object['status'] = 'OK';
				$return_object['html'] = '<div class="ulp-vision6-grouping" style="margin-bottom: 10px;"><strong>'.__('Invalid API Key or List ID.', 'ulp').'</strong></div>';
				echo json_encode($return_object);
				exit;
			}
			...
			$return_object = array();
			$return_object['status'] = 'OK';
			$return_object['html'] = $this->get_fields_html($key, $list, $this->default_popup_options['vision6_fields']);
			echo json_encode($return_object);
		}
		exit;
	}
	...
	function connect($_data = array()) {
		$headers = array(
			'Content-Type: application/json;charset=UTF-8',
			'Accept: application/json'
		);
		try {
			$url = 'https://app.vision6.com/api/jsonrpcserver?version=3.0';
			$curl = curl_init($url);
